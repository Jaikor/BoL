...
--[[
		...
		Version 5.0 updated by JaiKor for patch 6.21
...
function OnLoad()
	...
	Menu()
	loadOrbwalker()
	print("Pantheon - The Artisan of War v5.0 by Roach updated by JaiKor")
...
function EnableAttacks()
 ...
 function loadOrbwalker()
        if _G.Reborn_Loaded ~= nil then
            SAC = true
            PrintChat ("<font color=\"#F56527\">Found SAC Disabling SxOrb</font>")
        elseif _G.MMA_IsLoaded then
            MMA = true
            PrintChat ("<font color=\"#F56527\">Found MMA Disabling SxOrb</font>")
        elseif _G.NebelwolfisOrbWalkerInit then
            WOLFY = true
            PrintChat ("<font color=\"#F56527\">Found NebelwolfisOrbWalker Disabling SxOrb</font>")
        elseif _Pewalk then
            PEW = true
            PrintChat ("<font color=\"#F56527\">Found Pewalk Disabling SxOrb</font>")
        else
            SX = true
            PrintChat ("<font color=\"#F56527\">No Orbwalker found loaded Default: SxOrbWalk</font>")
		require 'SxOrbWalk'
                    PanthMenu:addSubMenu("["..myHero.charName.."] - Orbwalking Settings", "Orbwalking")
                    SxOrb:LoadToMenu(PanthMenu.Orbwalking, false)           
        end
        if not SAC and not SX and not MMA and not WOLFY and not PEW then
...
function GetCustomTarget()
	...
	if _G.MMA_IsLoaded and _G.MMA_Target() and _G.MMA_Target().type == myHero.type then 
            return _G.MMA_Target() 
        elseif _G.NebelwolfisOrbWalkerLoaded then 
           return _G.NebelwolfisOrbWalker:GetTarget()
        elseif _Pewalk then 
           return _Pewalk.GetTarget(1100)
